...
class AdamSLS(StochLineSearchBase):
    ...
    def step(self, closure):
     ...
     #   self.state['n_backwards'] += 1        
        ...
        # =================
        if self.first_step:
            step_size, loss_next = self.line_search(-1,step_sizes, params_current, grad_current, loss, closure_deterministic, grad_norm, non_parab_dec=pp_norm, precond=True)
        ...
        #      self.c  = self.c / len(step_sizes)
            ...
            for i,step_size in enumerate(step_sizes):
                if self.strategy == "impact_mag":
                    if rand == i:
                        step_size, loss_next = self.line_search(i,step_sizes, params_current, grad_current, loss, closure_deterministic, grad_norm, non_parab_dec=pp_norm, precond=True)
                        ...
                        self.try_sgd_precond_update(i,self.params, step_sizes, params_current, grad_current, self.momentum)
    ...
    @torch.no_grad()
    def try_sgd_precond_update(self, i,params, step_sizes, params_current, grad_current, momentum):
        if self.gv_option in ['scalar']:
            if i == -1:
                ...
                for p_next, p_current, g_current in zipped:
                        ...
                        p_next.data.add_(g_current, alpha=- step_sizes[0])
            else:
                for i, step_size in enumerate(step_sizes):
            ...
            if self.base_opt == 'adam':
                if i == -1:
                    ...
                    for p_next, p_current, g_current, gv_i, mv_i in zipped:
                            ...
                            p_next.data[:] = p_current.data
                            p_next.data.add_((pv_list *  mv_i_scaled), alpha=- step_sizes[0])
                else:
                    for i, step_size in enumerate(step_sizes):
                        zipped = zip(params[i], params_current[i], grad_current[i], self.state['gv'][i], self.state['mv'][i])
                        for p_next, p_current, g_current, gv_i, mv_i in zipped:
                            gv_i_scaled = scale_vector(gv_i, self.beta, self.state['step']+1)
                            pv_list = 1. / (torch.sqrt(gv_i_scaled) + 1e-8)
